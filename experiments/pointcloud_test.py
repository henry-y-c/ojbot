...
# d = 6.0
# for i in range(0,53):
#     pos[i] = np.random.random(size=(1, 3))
#     size[i] = 0.1
#     color[i] = (1.0, 1.0, 1.0, 0.5)
#     z *= 0.5
#     d *= 2.0
...
class PointCloudGraph:
    ...
    def __init__(self):
        ...
        self.sp = gl.GLScatterPlotItem(pos=self.pos, color=(1,1,1,0.5), size=0.1, pxMode=False)
...
def update():
    global graph
    graph.addData(np.random.random(size=(1, 3)))

graph = PointCloudGraph()
t = QtCore.QTimer()
...
#     return (True, [panAngle, tiltAngle, 100])

# def convertToCartesianCoord(sphereCoord):
#     return sphereCoord
...
# w.opts['distance'] = 20
# w.show()
# w.setWindowTitle('Test point cloud with pyqtgraph')

# while True:
#     time.sleep(0.1)
#     isRunning, data = nextFakeData()
#     # if isRunning:
